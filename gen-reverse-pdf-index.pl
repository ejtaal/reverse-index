...
#		print Dumper \@column_left;
		...
		foreach my $regex ( @bad_lines) {
			if ( m|$regex|) {
				...
				next LINE; # Get next $_ from <stdin>
			}
		}
		$text =~ s/<\/text>//;
		$text =~ s/<\/*(b|i)>//g;
		...
		if ( ( $lasttop - $top) > 250 or $page_non_garbage_line == 1) {
			if ( ($left - $lastleft) > 100 or $page_non_garbage_line == 1) {
				...
				#print "FLOAT OR WHAT?: ".(($left - $expected_left) / $avg_indent);
...
#				}
		...
		# New indent?
		elsif ( $page_non_garbage_line != 1 && abs( $top - $lasttop) > 7 && ($left - $lastleft > 5) ) {
...
#			}
		...
		# If start of this line is within 5 pix radius of end of last line assume it's continuation
			...
			# XXX
			# How to allow continuation of >Vitamin B<  >1< >pagenos<
			$width += $left - $lastleft;
			# Then adjust $left
...
#		}
		...
		$cur_outline_indent--;
		if ( $cur_outline_indent < 1 ) { $cur_outline_indent = 1; }
	}
	elsif ( m|^<outline| ) {
		$cur_outline_indent++;
	}
